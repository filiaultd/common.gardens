...
running script 16 with overwinter survival to look for relationship between proportion of genome and overwinter survival
...
fit.dat <- read.table("./data/ows.fitted.values.txt", stringsAsFactors=FALSE)
...
plot.fit.prop <- function(fit.col){
  par(mfcol=c(2,2))
  with(a.fit, plot(K9.ChrAll.3, get(fit.col), type="n", xlab="proportion genome N.Swedish", ylab=paste("ows",fit.col, sep=" ")))
  ...
  with(a.fit, plot(K9.ChrAll.6, get(fit.col), type="n", xlab="proportion of genome S.Swedish", ylab=paste("ows",fit.col, sep=" ")))
  ...
  with(a.fit, plot(K9.ChrAll.2, get(fit.col),type="n", xlab="proportion of genome German", ylab=paste("ows",fit.col, sep=" ")))
  ...
  with(a.fit, plot(1-(K9.ChrAll.3+K9.ChrAll.6), get(fit.col), type="n", xlab="proportion of genome other groups", ylab=paste("ows",fit.col, sep=" ")))
...
for(up.c in fit.columns){
  pdf(file=paste("./figures/ows.by.admix.prop.", up.c, ".pdf", sep=""), width=8, height=6)
  par(mar=c(5,4,1,1))
...
plot.slope <- function(up.s, up.p, up.t){
  ...
  up.p <- up.p[4:1,]
  par(mar=c(4,4,2,2))
  plot(0,0, xlim=c(min(up.s), max(up.s)), ylim=c(0.5,nrow(up.s)), yaxt="n", ylab="", xlab="slope ows~admix proportion", main=up.t)
...
pdf("./figures/slope.ows.admixture.pdf", width=10, height=6)
par(mfcol=c(1,2))
plot.slope(up.s=slope1, up.p=pval1, up.t="2011")
plot.slope(up.s=slope2, up.p=pval2, up.t="2012")
dev.off()
...
pdf(file="./figures/ows.rank.by.admix.prop.pdf", width=18, height=8)
...
#### Test for simple effect of admixture group on ows, using lines that are "well-assigned" to admixture groups (i.e. not considered admixed in 1001g data)

```{R admixture group ows diffs}
...
colnames(ga.long)[4] <- "ows"
ga.long$site = factor(ga.long$site,levels(ga.long$site)[c(1,2,4,3)])


ag.colors <- brewer.pal(9, "Paired")[c(3,4,7)]

ad.group.boxplot <- ggplot(ga.long, aes(x=as.factor(a.group), y=ows, fill=as.factor(a.group))) +
  geom_violin() +
...
pdf(file="./figures/ows.by.admix.group.pdf", width=8, height=6)
print(ad.group.boxplot)
dev.off()
...
Just to note, yes, the outlier accession for OWS in Ull and Rat 2012 is the same: 6900
