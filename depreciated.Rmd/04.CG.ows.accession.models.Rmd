---
title: "04.CG.ows.accession.models"
...
Taking a closer look at ows in the common garden experiments.
Taking same approach as for fitness and fecundity
Look for local adaptation and try to assess which components of ows
vary across sites and years to cause fitness differences.

Fit models to get ows for each site
First make data variable

## 1. Local adaptation with ows values - combined years
### 1a. prep data
```{r}
...
```
So the distribution of the ows variable clearly binary - use glm with binomial/logit link function
...
glm.u1 <- glm(ows ~ id + year + id*year + year/block, family=binomial, data=dat.up)

glm.u2 <- glm(ows ~ id + year + year/block, family=binomial, data=dat.up)
anova(glm.u2, glm.u1, test="Chisq")

glm.u3 <- glm(ows ~ id + year/block, family=binomial, data=dat.up)
anova(glm.u3, glm.u1, test="Chisq")

glm.u4 <- glm(ows ~ id + year + id*year, family=binomial, data=dat.up)
anova(glm.u4, glm.u1, test="Chisq")

glm.u5 <- glm(ows ~ year + year/block, family=binomial, data=dat.up)
anova(glm.u5, glm.u1, test="Chisq")

```

OK, this is a mess.  Let's try to build model up instead of deconstructing.

```{r}

glm.u6 <- glm(ows ~ id, family=binomial, data=dat.up)

glm.u7 <- glm(ows ~ 1, family=binomial, data=dat.up)
anova(glm.u6, glm.u7, test="Chisq")

glm.u8 <- glm(ows ~ id + year, family=binomial, data=dat.up)
anova(glm.u8, glm.u6, test="Chisq")

glm.u9 <- glm(ows ~ id + year + id* year, family=binomial, data=dat.up)
anova(glm.u8, glm.u9, test="Chisq")  # not significant

glm.u10 <- glm(ows ~ id + year +year/block, family=binomial, data=dat.up)
anova(glm.u8, glm.u10, test="Chisq")  # block effect significant

glm.u11 <- glm(ows ~ id + year + id*year +year/block, family=binomial, data=dat.up)
anova(glm.u11, glm.u10, test="Chisq")  # not significant
```

The best model is glm.a10, which includes id, year, and block nested in year.



```{r}
du.predict <- predict(glm.u10, type="response")
...
hist(du.means$du.predict, main="predicted means Ullstorp", xlab="ows")
...
summary(lm.ufit)
plot(du.means$lat, du.means$du.predict, xlab="latitude of accession origin", ylab="ows in Ullstorp")
...
```{r}
da <- d[d$exp=="ADA",]
hist(da$ows, main="Adal ows", xlab="ows")
```
...
glm.a6 <- glm(ows ~ id, family=binomial, data=dat.up)

glm.a7 <- glm(ows ~ 1, family=binomial, data=dat.up)
anova(glm.a6, glm.a7, test="Chisq")

glm.a8 <- glm(ows ~ id + year, family=binomial, data=dat.up)
anova(glm.a8, glm.a6, test="Chisq")

glm.a9 <- glm(ows ~ id + year + id* year, family=binomial, data=dat.up)
anova(glm.a8, glm.a9, test="Chisq")  # not significant

glm.a10 <- glm(ows ~ id + year +year/block, family=binomial, data=dat.up)
anova(glm.a8, glm.a10, test="Chisq")  # block effect significant

glm.a11 <- glm(ows ~ id + year + id*year +year/block, family=binomial, data=dat.up)
anova(glm.a11, glm.a10, test="Chisq")  # not significant
```

Again, model 10 is best


```{r}
da.predict <- predict(glm.a10, type="response")
hist(da.predict)
...
summary(lm.afit)
plot(da.means$lat, da.means$da.predict, xlab="latitude of accession origin", ylab="ows in Ådal East")
...
```{r}
dr <- d[d$exp=="RAT",]
hist(dr$ows, main="histogram of Rat ows")
```
...
glm.r6 <- glm(ows ~ id, family=binomial, data=dat.up)

glm.r7 <- glm(ows ~ 1, family=binomial, data=dat.up)
anova(glm.r6, glm.r7, test="Chisq")

glm.r8 <- glm(ows ~ id + year, family=binomial, data=dat.up)
anova(glm.r8, glm.r6, test="Chisq")

glm.r9 <- glm(ows ~ id + year + id* year, family=binomial, data=dat.up)
anova(glm.r8, glm.r9, test="Chisq")  # not significant

glm.r10 <- glm(ows ~ id + year +year/block, family=binomial, data=dat.up)
anova(glm.r8, glm.r10, test="Chisq")  # block effect significant

glm.r11 <- glm(ows ~ id + year + id*year +year/block, family=binomial, data=dat.up)
...
```{r}
dr.predict <- predict(glm.r10, type="response")
...
summary(lm.rfit)
plot(dr.means$lat, dr.means$dr.predict, xlab="latitude of accession origin", ylab="ows in Ratheckgården")
...
```{r}
dm <- d[d$exp=="RAM",]
hist(dm$ows, main="histogram of Ramsta ows")
```
...
glm.m6 <- glm(ows ~ id, family=binomial, data=dat.up)

glm.m7 <- glm(ows ~ 1, family=binomial, data=dat.up)
anova(glm.m6, glm.m7, test="Chisq")

glm.m8 <- glm(ows ~ id + year, family=binomial, data=dat.up)
anova(glm.m8, glm.m6, test="Chisq")

glm.m9 <- glm(ows ~ id + year + id* year, family=binomial, data=dat.up)
anova(glm.m8, glm.m9, test="Chisq")  # not significant

glm.m10 <- glm(ows ~ id + year +year/block, family=binomial, data=dat.up)
anova(glm.m8, glm.m10, test="Chisq")  # block effect significant

glm.m11 <- glm(ows ~ id + year + id*year +year/block, family=binomial, data=dat.up)
...
summary(lm.mfit)
plot(dm.means$lat, dm.means$dm.predict, xlab="latitude of accession origin", ylab="ows in Ramsta")
...
with(all.means, plot(s.means, n.means, xlab="mean ows in South", ylab="mean ows in North"))
#abline(lm.ns.mean)
abline(0,1, col="blue")
...
ns.plot + theme_bw() +   geom_smooth( aes(linetype = region, color=region), method = "lm") + labs(x="mean ows in Southern experiments", y="mean ows in Northern experiments")

lm.nfit <- lm(n.means~lat, data=all.means)
summary(lm.nfit)
plot(all.means$lat, all.means$n.means, xlab="latitude of accession origin", ylab=" mean ows in Northern sites")
abline(lm(n.means~lat, data=all.means), col="blue")

lm.sfit <- lm(s.means~lat, data=all.means)
summary(lm.sfit)
plot(all.means$lat, all.means$s.means, xlab="latitude of accession origin", ylab=" mean ows in Southern sites")
abline(lm(s.means~lat, data=all.means), col="blue")

```
1.  Joint years, local adaptation in S, not in N.
2.  generally higher ows in N

## 3. Local adaptation with ows values - 2011
...
glm.u11 <- glm(ows ~ 1, family=binomial, data=dat.up)

glm.u12 <- glm(ows ~ id, family=binomial, data=dat.up)
anova(glm.u12, glm.u11, test="Chisq")

glm.u13 <- glm(ows ~ id + block, family=binomial, data=dat.up)
...
```{r}
du1.predict <- predict(glm.u13, type="response")
...
summary(lm.u1fit)
plot(du1.means$lat, du1.means$du1.predict, xlab="latitude of accession origin", ylab="ows in Ullstorp 2011")
...
glm.a11 <- glm(ows ~ 1, family=binomial, data=dat.up)

glm.a12 <- glm(ows ~ id, family=binomial, data=dat.up)
anova(glm.a12, glm.a11, test="Chisq")

glm.a13 <- glm(ows ~ id + block, family=binomial, data=dat.up)
...
summary(lm.a1fit)
plot(da1.means$lat, da1.means$da1.predict, xlab="latitude of accession origin", ylab="ows in Ådal East 2011")
...
glm.r11 <- glm(ows ~ 1, family=binomial, data=dat.up)

glm.r12 <- glm(ows ~ id, family=binomial, data=dat.up)
anova(glm.r12, glm.r11, test="Chisq")

glm.r13 <- glm(ows ~ id + block, family=binomial, data=dat.up)
...
summary(lm.r1fit)
plot(dr1.means$lat, dr1.means$dr1.predict, xlab="latitude of accession origin", ylab="ows in Rathckegården 2011")
...
glm.m11 <- glm(ows ~ 1, family=binomial, data=dat.up)

glm.m12 <- glm(ows ~ id, family=binomial, data=dat.up)
anova(glm.m12, glm.m11, test="Chisq")

glm.m13 <- glm(ows ~ id + block, family=binomial, data=dat.up)
...
summary(lm.m1fit)
plot(dm1.means$lat, dm1.means$dm1.predict, xlab="latitude of accession origin", ylab="ows in Ramsta 2011")
...
with(all.means1, plot(s.means, n.means, xlab="mean ows in South", ylab="mean ows in North"))
#abline(lm.ns.mean)
abline(0,1, col="blue")
...
ns.plot2011 + theme_bw() +   geom_smooth( aes(linetype = region, color=region), method = "lm") + labs(x="mean ows in Southern experiments 2011", y="mean ows in Northern experiments 2011")
```
...
summary(lm.n1)
plot(all.means1$lat, all.means1$n.means, xlab="latitude of accession origin", ylab="mean ows in North 2011")
abline(lm(n.means~lat, data=all.means1), col="blue")

lm.s1 <- lm(s.means~lat, data=all.means1)
summary(lm.s1)
plot(all.means1$lat, all.means1$s.means, xlab="latitude of accession origin", ylab="mean ows in South 2011")
...
## 4. Local adaptation with ows values - 2012
...
glm.u21 <- glm(ows ~ 1, family=binomial, data=dat.up)

glm.u22 <- glm(ows ~ id, family=binomial, data=dat.up)
anova(glm.u22, glm.u21, test="Chisq")

glm.u23 <- glm(ows ~ id + block, family=binomial, data=dat.up)
...
Survival at Ullstorp is just nearly 100%.
Use model three (which is marginally significant) anyway.

 
```{r}
du2.predict <- predict(glm.u23, type="response")
...
summary(lm.u2fit)
plot(du2.means$lat, du2.means$du2.predict, xlab="latitude of accession origin", ylab="ows in Ullstorp 2012")
...
glm.a21 <- glm(ows ~ 1, family=binomial, data=dat.up)

glm.a22 <- glm(ows ~ id, family=binomial, data=dat.up)
anova(glm.a22, glm.a21, test="Chisq")

glm.a23 <- glm(ows ~ id + block, family=binomial, data=dat.up)
...
As in Ullstorp, nothing significant because survival is high.
Just use model 2
...
summary(lm.a2fit)
plot(da2.means$lat, da2.means$da2.predict, xlab="latitude of accession origin", ylab="ows in Ådal East 2012")
...
glm.r21 <- glm(ows ~ 1, family=binomial, data=dat.up)

glm.r22 <- glm(ows ~ id, family=binomial, data=dat.up)
anova(glm.r22, glm.r21, test="Chisq")

glm.r23 <- glm(ows ~ id + block, family=binomial, data=dat.up)
...
Nothing significant - high survival
Just use model 2
...
summary(lm.r2fit)
plot(dr2.means$lat, dr2.means$dr2.predict, xlab="latitude of accession origin", ylab="ows in Rathckegården 2012")
...
glm.m21 <- glm(ows ~ 1, family=binomial, data=dat.up)

glm.m22 <- glm(ows ~ id, family=binomial, data=dat.up)
anova(glm.m22, glm.m21, test="Chisq")

glm.m23 <- glm(ows ~ id + block, family=binomial, data=dat.up)
anova(glm.m23, glm.m22, test="Chisq")

glm.m24 <- glm(ows ~ block, family=binomial, data=dat.up)
...
Again, not a lot significant.  Use model 3 (which is better than model 2 - anova also shows some line effects)
...
summary(lm.m2fit)
plot(dm2.means$lat, dm2.means$dm2.predict, xlab="latitude of accession origin", ylab="ows in Ramsta 2012")
...
with(all.means2, plot(s.means, n.means, xlab="mean ows in South", ylab="mean ows in North"))
#abline(lm.ns.mean)
abline(0,1, col="blue")
...
ns.plot2012 + theme_bw() +   geom_smooth( aes(linetype = region, color=region), method = "lm") + labs(x="mean ows in Southern experiments 2012", y="mean ows in Northern experiments 2012")
```
ows much higher in the North than the South, N and S accessions ows the same
...
summary(lm.n2)
plot(all.means2$lat, all.means2$n.means, xlab="latitude of accession origin", ylab="mean ows in North 2012")
abline(lm(n.means~lat, data=all.means2), col="blue")

lm.s2 <- lm(s.means~lat, data=all.means2)
summary(lm.s2)
plot(all.means2$lat, all.means2$s.means, xlab="latitude of accession origin", ylab="mean ows in South 2012")
abline(lm(s.means~lat, data=all.means2), col="blue")
```
So, relationship between latitude and ows is significant in both N and S, but in N, it goes the wrong direction!
...
From the above, it looks like ows patterns are different for the two years, as well as between sites so I want to look at this more closely.
...
hist(acc.fit$ows, main="predicted mean ows, all sites, both years")
with(acc.fit, boxplot(ows~year+site, xlab="year and site",ylab="ows", las=2))
```
...
```{r}
lm.b1 <- lm(ows~id + year + site + year*site, data=acc.fit)
anova(lm.b1)

lm.b2 <- lm(ows~id + year + id*year + site + id*site + year*site, data=acc.fit)
anova(lm.b2)
...
ns.plot.both + theme_bw() +   geom_smooth( aes(linetype = year, color= region), method = "lm") + labs(x="mean ows in Southern experiments", y="mean ows in Northern experiments")
```
...
ows.fitted.values <- merge(all.min, all1.min, by="id", all=TRUE)
ows.fitted.values <- merge(ows.fitted.values, all2.min, by="id", all=TRUE)
write.table(ows.fitted.values, file="./data/ows.fitted.values.txt", quote=FALSE)
```
...
ows.adapt.models <- as.list(1:8)
for(up in 1:8){ows.adapt.models[[up]] <- get(models[up])}
save(ows.adapt.models, file="./data/ows.adapt.models.Rdat")
```

## 8.  Summary
ows is only a player in 2011
Here, it is locally adaptive in Ramsta, and opposite-nonadaptive in Rat
