...
fit.dat <- read.table("./data/sss.fitted.values.txt", stringsAsFactors=FALSE)
...
plot.fit.prop <- function(fit.col){
  par(mfcol=c(2,2))
  with(a.fit, plot(K9.ChrAll.3, get(fit.col), type="n", xlab="proportion genome N.Swedish", ylab=paste("sss",fit.col, sep=" ")))
  ...
  with(a.fit, plot(K9.ChrAll.6, get(fit.col), type="n", xlab="proportion of genome S.Swedish", ylab=paste("sss",fit.col, sep=" ")))
  ...
  with(a.fit, plot(K9.ChrAll.2, get(fit.col),type="n", xlab="proportion of genome German", ylab=paste("sss",fit.col, sep=" ")))
  ...
  with(a.fit, plot(1-(K9.ChrAll.3+K9.ChrAll.6), get(fit.col), type="n", xlab="proportion of genome other groups", ylab=paste("sss",fit.col, sep=" ")))
...
for(up.c in fit.columns){
  pdf(file=paste("./figures/sss.by.admix.prop.", up.c, ".pdf", sep=""), width=8, height=6)
  par(mar=c(5,4,1,1))
...
plot.slope <- function(up.s, up.p, up.t){
  ...
  up.p <- up.p[4:1,]
  par(mar=c(4,4,2,2))
  plot(0,0, xlim=c(min(up.s), max(up.s)), ylim=c(0.5,nrow(up.s)), yaxt="n", ylab="", xlab="slope sss~admix proportion", main=up.t)
...
pdf("./figures/slope.sss.admixture.pdf", width=10, height=6)
par(mfcol=c(1,2))
plot.slope(up.s=slope1, up.p=pval1, up.t="2011")
plot.slope(up.s=slope2, up.p=pval2, up.t="2012")
dev.off()
...
pdf(file="./figures/sss.rank.by.admix.prop.pdf", width=18, height=8)
...
dev.off()
```
#### Test for simple effect of admixture group on sss, using lines that are "well-assigned" to admixture groups (i.e. not considered admixed in 1001g data)
...
colnames(ga.long)[4] <- "sss"
ga.long$site = factor(ga.long$site,levels(ga.long$site)[c(1,2,4,3)])


ag.colors <- brewer.pal(9, "Paired")[c(3,4,7)]

ad.group.boxplot <- ggplot(ga.long, aes(x=as.factor(a.group), y=sss, fill=as.factor(a.group))) +
  geom_violin() +
...
pdf(file="./figures/sss.by.admix.group.pdf", width=8, height=6)
print(ad.group.boxplot)
dev.off()
