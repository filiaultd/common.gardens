...
```{r linear models with Kgroups}
dk <-merge(d, kg, all.x=TRUE)

## model with k.2 instead of genotype ()
dk$k.2 <- as.factor(dk$k.2)
lm.k2 <- lm(fitness~k.2*year*exp + exp:year/block, data=dk)
anova(lm.k2)

## model with k.6 instead of genotype
dk$k.6 <- as.factor(dk$k.6)
lm.k6 <- lm(fitness~k.6*year*exp + exp:year/block, data=dk)
anova(lm.k6)
## k.6 is much more significant than k.2!

ssq2 <- anova(lm.k2)[[2]]
pve2 <-(ssq2/sum(ssq2))*100
names(pve2) <- rownames(anova(lm.k2))

ssq6 <- anova(lm.k6)[[2]]
pve6 <-(ssq6/sum(ssq6))*100
names(pve6) <- rownames(anova(lm.k6))
